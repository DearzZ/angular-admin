...
app.controller('resourceFollowCtrl', ['$scope','$cookies','$cookieStore','$modal','toaster','$http',function($scope,$cookies,$cookieStore,$modal,toaster,$http) {
    //  vm -----  页面控制对象；；；；；；；；；；    bs  传输数据对象
    var vm = $scope.vm = {};
    var bs = $scope.bs = {};
    //   默认选中显示姓名
...
//   设置显示字段不设定只能点击 X 关闭
    ...
    vm.setShowCookie=function () {
        $cookieStore.remove('resourceFollowTableList');
        var expireDate = new Date();
        expireDate.setDate(expireDate.getSeconds() + 100);
        $cookieStore.put("resourceFollowTableList", vm.showTabel,{'expires': expireDate.toUTCString()});
        vm.setTableWidth('#show_table');
    };
    //  页面加载 初始化 设置显示字段
    vm.showTabel =  angular.fromJson($cookies['resourceFollowTableList']) || vm.showTabel_default;
    ...
    vm.openConnect =function (currentId) {
        var modalInstance = $modal.open({
            templateUrl: 'resourceFollowCurrent.html',
